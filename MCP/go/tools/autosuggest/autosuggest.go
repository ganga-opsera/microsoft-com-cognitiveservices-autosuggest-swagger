...
func AutosuggestHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["ResponseFormat"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("ResponseFormat=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/Suggestions%s", cfg.BaseURL, queryString)
...
func CreateAutosuggestTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_Suggestions",
		mcp.WithDescription("The AutoSuggest API lets you send a search query to Bing and get back a list of query suggestions. This section provides technical details about the query parameters and headers that you use to request suggestions and the JSON response objects that contain them."),
		...
		mcp.WithArray("ResponseFormat", mcp.Description("The media type to use for the response. The following are the possible case-insensitive values: JSON, JSONLD. The default is JSON. If you specify JSONLD, the response body includes JSON-LD objects that contain the search results.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    AutosuggestHandler(cfg),
	}
}
